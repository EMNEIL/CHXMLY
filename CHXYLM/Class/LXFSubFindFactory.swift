...
class LXFSubFindFactory: NSObject {
    // MARK:- 生成子控制器
    class func subFindVcWith(identifier: String ) -> CHBaseViewController {
        let subFindType: LXFSubFindType = typeFromTitle(identifier)
        var controller: CHBaseViewController!
        switch subFindType {
        case .LXFSubFindTypeRecommend:
            controller = CHRecommandViewController()
        ...
        case .LXFSubFindTypeAnchor:
            controller = CHAnchorViewController()
        default:
            controller = CHBaseViewController()
        }
        return controller
    }
